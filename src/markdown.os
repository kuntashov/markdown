...
Функция ВызватьОбработчикБлока(ТипБлока, ТипОбработчика, Аргумент1=Неопределено, Аргумент2=Неопределено)
	...
	Возврат "Обработчик для типа блока " + ТипБлока + " не реализован";
КонецФункции
...
Функция БлокList(СтруктураСтроки)
	...
	Если ЧастиСтроки.Количество() > 1 Тогда
		...
		Блок.Вставить("element", Новый Структура("name,handlers", Имя, "elements"));
		...
		ТекстПунктаСписка = Сред(СтруктураСтроки.text, СтрДлина(ЧастиСтроки[0]));
		
		Блок.Вставить("li", Новый Структура("name,handler,text", "li", "li", ММ(ТекстПунктаСписка)));
...
Функция ВызватьОбработчикИнлайна(ТипИнлайна, СтруктураПодстроки)
	...
	Возврат "Обработчик инлайна " + ТипИнлайна + " не реализован";
КонецФункции
...
Функция ИнлайнEmphasis(СтруктураПодстроки)
	...
	Инлайн.Вставить("extent", СтрДлина(СтрСодержимое) + СтрДлина(Маркер));
...
Функция СодержитАкцент(Знач Текст, Знач Маркер, СтрСодержимое)
	...
	СтрСодержимое = Сред(Текст, ДлинаМаркера + 1, ПозКонца - ДлинаМаркера);
...
Функция ВывестиСообщение(Знач ТекстСообщения, П1=Неопределено,П2=Неопределено,П3=Неопределено,П4=Неопределено,П5=Неопределено,П6=Неопределено,П7=Неопределено,П8=Неопределено,П9=Неопределено)
	...
	Для Н=1 По П.Количество() - 1 Цикл
		Если П[Н] = Неопределено Тогда
			Прервать;
		КонецЕсли;
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%" + Строка(Н), П[Н]);
	КонецЦикла;
		
	Сообщить(ТекстСообщения);
КонецФункции

Функция Отладка(Знач Объект, Заголовок="", Отступ="")
	
	Результат = "";
		
	Если СтрДлина(Отступ) > 100 Тогда
		Возврат Результат;
	КонецЕсли;
	
	Если НЕ ПустаяСтрока(Заголовок) И СтрДлина(Отступ) = 0 Тогда
		Результат = Результат + ВывестиСообщение("======================= %1 =======================", Заголовок);
	КонецЕсли;
	
	Если ТипЗнч(Объект) = Тип("HTTPОтвет") Тогда
		Результат = Результат + ВывестиСообщение("КодСостояния: %1", Объект.КодСостояния);
		Результат = Результат + ВывестиСообщение("------- Заголовки: ");
		Результат = Результат + Отладка(Объект.Заголовки, "", "    ");
		Результат = Результат + ВывестиСообщение("------- Тело: ");
		Результат = Результат + Отладка(Объект.ПолучитьТелоКакСтроку(),"", "    ");
	ИначеЕсли ТипЗнч(Объект) = Тип("Структура") или ТипЗнч(Объект) = Тип("Соответствие") Тогда
		Для каждого Элемент из Объект Цикл
			Результат = Результат + ВывестиСообщение("%1 %2: %3", Отступ, Элемент.Ключ, Элемент.Значение);
			Если ТипЗнч(Элемент.Значение) = Тип("Структура") 
			 или ТипЗнч(Элемент.Значение) = Тип("Соответствие") 
			 или ТипЗнч(Элемент.Значение) = Тип("Массив") Тогда
				Результат = Результат + Отладка(Элемент.Значение, Отступ + "    ");
			КонецЕсли;
		КонецЦикла;	
	ИначеЕсли ТипЗнч(Объект) = Тип("Массив") Тогда
			Результат = Результат + ВывестиСообщение(Отступ + "Массив(");
			Для каждого ЭлементМассива Из Объект Цикл
				Результат = Результат + Отладка(ЭлементМассива, Заголовок, Отступ + "    ");
				Результат = Результат + ВывестиСообщение(Отступ + "=================");
			КонецЦикла;
		Результат = Результат + ВывестиСообщение(Отступ + ")");
	Иначе
		Результат = Результат + ВывестиСообщение(Отступ + Строка(Объект));
	КонецЕсли;	
